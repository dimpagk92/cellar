...
| **Tier B1 (infrastructure)** | `MemorySelector` trait + read-time re-rank + always-safe fallback to WK1 deterministic ordering | ✅ merged (#42) — +4 runner tests + 3 trait tests; mirrors A4 + WK2 pattern (seam shipped, no bundled LLM impl) |
| **Closing-gap fill** | `view.evidence` synthesized from selected memories/knowledge/events/adapter_facts; `view.adapter_facts` populated via new `AdapterDriver::facts_for_planning_view` + Cortex aggregator + `StepExecutor::adapter_facts` | 📝 PR'd — gates green, +4 builder tests; closes the two PlanningView fields originally listed in the acceptance criteria but missed by the post-WK reframe |
| ~~Tier B3~~ | ~~Cross-workflow priors~~ | **out of scope for cellar** — workflow is a caller-defined string in cellar; cross-workflow grouping is the agent/host's concern. CEL can add a `list_for_workflows` (plural) read API in ~1 hour if/when a real consumer asks |
...
| ⏳ **Next** | Production dogfooding / platform breadth (adapters, benchmark server) | strategic call — next chapter, not in this plan |
...
- [x] **Closing-gap fill**: `view.evidence` synthesized from selected memories/knowledge/events/adapter_facts. `view.adapter_facts` populated by the runner via the new `AdapterDriver::facts_for_planning_view` trait method (default empty), the new `Cortex::collect_adapter_facts_for_planning_view` aggregator, and the new `StepExecutor::adapter_facts` method. Closes the two PlanningView fields originally listed in the acceptance criteria but missed by the post-WK reframe. *(Shipped this PR; 4 new builder tests pin the contract.)*
- [x] ~~**Tier B3**: Cross-workflow priors~~ — **reclassified as out of scope for cellar.** Workflow is a caller-defined string in cellar; cross-workflow grouping is the agent/host's concern (cellar, LangGraph, MCP clients, ...). CEL would add a `list_for_workflows` (plural) read API in ~1 hour if/when a real consumer asks; until then no engineering work is needed.
