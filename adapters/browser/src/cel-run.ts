...
async function runGoalWithRustFallback(
  ...
  try {
    ...
    const rustResult = await cel.runGoalRust({
      ...
      enable_vision: config.enableVision ?? true,
      self_heal: config.selfHeal ?? true,
      enable_decomposition: config.enableDecomposition ?? false,
      enable_notebook: config.enableNotebook ?? true,
