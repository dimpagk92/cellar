//! Canonical agent contract — the single set of types every caller
//! (CLI, MCP server, eval harness, benchmarks) speaks.
//!
//! See `docs/canonical-agent-plan.md` for the motivating plan. In short:
//! the old code has two parallel paths (the CLI runner and the eval
//! harness each drive the agent differently) with a knob for every
//! variation. This module is the new shape they both collapse onto:
//!
//!   * `Plan`          — what the planner returns for a goal
//!   * `SubGoal`       — one ordered chunk of work inside a plan
//!   * `Step`          — one executable unit inside a sub-goal
//!   * `StepResult`    — outcome of a single attempt
//!   * `GoalOutcome`   — terminal result of a whole run
//!   * `FailureReport` — structured explanation for the 3-strike rule
//!
//! These types are deliberately minimal. Each new field would be a new
//! knob that the CLI/eval/MCP paths could drift on. Add one only when a
//! caller genuinely needs it, and add it in one place.
...
/// One executable unit inside a [`SubGoal`].
///
...
pub struct Step {
    ...
    /// The action to execute. Reuses the existing [`PlannedAction`]
    /// enum so the refactor does not simultaneously touch the action
    /// type set.
    pub action: PlannedAction,
}
...
pub enum StepResult {
    ...
    Ok {
        ...
        #[serde(default)]
        discovered_sub_goal: Option<SubGoal>,
    },
...
/// Terminal outcome of a whole [`GoalRunner::run`](crate) invocation.
///
...
/// Budget limits for a single `GoalRunner::run` invocation.
///
