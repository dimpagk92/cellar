...
/// Blanket impl so the daemon can hand an `Arc<W>` (typically
/// `Arc<SqliteRulesStore>`) to both the gateway and the matcher
/// consumer task. Without this, calling sites that share one store
/// across both consumers can't compile.
impl<T: WatchlistLookup + ?Sized> WatchlistLookup for std::sync::Arc<T> {
    fn contains(&self, watchlist_name: &str, item: &str) -> bool {
        (**self).contains(watchlist_name, item)
    }
}
