...
impl<S: Sender + 'static> WebhookService<S> {
    ...
    pub fn spawn(
        ...
        tokio::spawn(async move {
            ...
            while let Some(job) = rx.recv().await {
                let cfg_ref = match webhooks_for_worker.get(&job.webhook_id) {
                    Some(c) => c,
                    None => {
                        tracing::warn!(
                ...
                let secret = secrets_for_worker.get(&job.webhook_id);
                let result = dispatcher
                    .dispatch(&job.rule_id, cfg_ref, secret, &job.payload)
                    .await;
                on_result(result);
            }
        });
    ...
    pub fn webhook_ids(&self) -> Vec<&str> {
        self.webhooks.keys().map(String::as_str).collect()
    }
